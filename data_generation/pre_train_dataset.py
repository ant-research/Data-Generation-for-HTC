...
class NLGMixSenClsDataset(Dataset):
    ...
    def __init__(self, config, data_path, tokenizer, label_index, is_training=False):
        ...
        self.yake = yake.KeywordExtractor(lan=language, 
                                            n=max_ngram_size, 
        ...
        self.data_list = []
        self.label_dict = self.get_label_dict(self.config.label_path)
        self.label_keywords_dict = []
        self.label_keywords_dict = self.get_label_keywords_dict()
        self.label_tree = None
        self.keywords_dict = pickle.load(open('./wos_keywords_v4_25.pkl','rb'))


        with open(self.config.label_tree_path,'r') as f:
            self.label_tree = json.load(f)
        ...
        if self.is_training:
            # self.mode_func = {
            #     "keyword": self.gen_from_keyword_sequence_train,
            ...
            # }          
            self.mode_func = {
                "keyword": self.gen_from_keyword_sequence_hier,
                "tag": self.gen_from_tag_sequence,
            }                   
        else:
            # self.mode_func = {
        ...
        for i in range(len(lines)):
            ...
            self.keyword_list.append(line)
        # self.keyword_list = pickle.load(open('./keywords_balance_att_1.pkl','rb'))
            ...
            with open(data_path) as out:
                ...
                for l in out:
                    ...
                    for _ in range(config.eval_data_replication if not is_training else 1):
                        self.data_list.append(('\t'.join(items[:-1]), items[-1]))
                    for w in set('\t'.join(items[:-1]).split()):     
                        w = w.lower()
            ...
            with open(data_path) as out:
                ...
                for l in out:
                    ...
                    for w in set(combined_sen.split()):
                        ...
                        vocab2doc[w].append(doc_id)
                    doc_id += 1
        # self.sample_prob = self.get_sample_prob_wos(self.label_dict)
        self.idf_value = {}
        for w in vocab2doc:
    ...
    def get_label_keywords_dict(self):
        '''
            output:
                label_keywords_dict -> dict{label:[keywords of this class]}
        '''
        path = self.config.keywords_dict_path
        ...
        for i in range(len(lines)):
            ...
            label_keywords_dict[label] = [word[0] for word in line[label]]
        return label_keywords_dict
                

    def gen_from_tag_sequence(self, token, tag):
        
        input_y = ' and '.join(tag.split(', ')) + " <extra_id_0> " + token
        input_x = ' and '.join(tag.split(', '))
    ...
    def gen_from_keyword_sequence_json(self, line):


        input_y = line['text']
        tag = line['label_name']
        # mention_list = line['keywords_att'].split()
        mention_list = line['keywords_yake'].split(',')
        # random.shuffle(mention_list)

        # input_x =' '.join(' and '.join(mention_list).split()[:100])  # "generate with keywords: " + 
        # input_x = 'generate with keywords: : '+' , '.join(mention_list)+ '; generate with labels: '+tag  # "generate with keywords: " + 
        input_x = 'generate with labels: ' +', '.join(tag)+ '; generate with keywords: '+' , '.join(mention_list)  # "generate with keywords: " + 
    ...
    def gen_from_keyword_sequence_hier(self, token, tag):
        input_y =token



        raw_keyword_list = self.keywords_dict[token+'\t'+tag+'\n'].strip('\n').split()

        if len(raw_keyword_list) > 3:
            mention_list = raw_keyword_list
        else:
            ...
            mention_list = raw_keyword_list+ [k[0] for k in w_idf[:4]]
        ...
        # rake_keyword_list = self.r.get_ranked_phrases()
        # if len(rake_keyword_list) > 0:
        #     rake_mention_list = clean_top_features(rake_keyword_list, top=5)

        #     if len(rake_mention_list) > 0:
        #         mention_list = mention_list[:5] + rake_mention_list
        #         random.shuffle(mention_list)

        # input_x =' '.join(' and '.join(mention_list).split()[:100])  # "generate with keywords: " + 
        # input_x = 'generate with keywords:'+' , '.join(mention_list)+ '; generate with labels: '+tag  # "generate with keywords: " + 
        input_x = 'generate with labels: ' +tag+ '; generate with keywords: '+' , '.join(mention_list)  # "generate with keywords: " + 
    ...
    def gen_from_keyword_sequence_train(self, token, tag):
        input_y =token
        self.r.extract_keywords_from_text(token)
    ...
    def gen_from_keyword_sequence_yake(self, token, tag):
        input_y =token

        keywords = self.yake.extract_keywords(token)
        raw_keyword_list = [item[0] for item in keywords]        
        # if len(raw_keyword_list) > 0:
        #     mention_list = clean_top_features(raw_keyword_list, top=5)
        # else:
        #     current_tokens = token.split()
        ...
        fa_label = self.label_tree[label]

        if self.config.gen_templete == 'class_keywords':#keywords = hybrid(key(x),key(tags))
            external_keywords = self.label_keywords_dict[label][:5]
            raw_keyword_list = raw_keyword_list + external_keywords

        if self.config.gen_templete == 'father_class_ketwords':
            external_keywords = self.label_keywords_dict[fa_label][:5]
            raw_keyword_list = raw_keyword_list + external_keywords

        if self.config.gen_templete == "hybrid":
            external_keywords = self.label_keywords_dict[fa_label][:5] + self.label_keywords_dict[label][:5]
            raw_keyword_list = raw_keyword_list + external_keywords
        no_keywords_flag = False
        if self.config.gen_templete == "rand":
            ...
            if randnum == 3:
                ...
                raw_keyword_list = raw_keyword_list + external_keywords
            if randnum == 4:
                no_keywords_flag = True

        # if self.config.gen_templete == 
        if no_keywords_flag == False:
            if len(raw_keyword_list) > 0:
                mention_list = clean_top_features(raw_keyword_list, top=10)
            else:
                current_tokens = token.split()
                w_idf = [(w, self.idf_value[w.lower()]) for w in current_tokens if w.lower() in self.idf_value]
                w_idf = sorted(w_idf, key=lambda x: x[1], reverse=True)
                mention_list = [k[0] for k in w_idf[:11]]
            random.shuffle(mention_list)
            ...
            input_x = 'generate with labels: ' + tag
    ...
    def get_label_dict(self,path=None):
        labels = []
        if path == None:
            with open("./NLU_training_dataset/WebOfScience/labels.txt","r") as f:
                labels = f.readlines()
        else:
            with open(path,"r") as f:
                labels = f.readlines()
        ...
        for i in range(len(labels)):
            label_dict[labels[i].strip()] = i
        return label_dict

    def get_sample_prob_wos(self,dict):
        cnt = {}
        for i in range(len(self.data_list)):
            label = self.data_list[i][1].split(',')[1].strip()
            label_id = dict[label]
            if label_id not in cnt:
                cnt[label_id] = 1
            else:
                cnt[label_id] += 1         
        prob = [0 for i in range(len(self.data_list))]

        for i in range(len(self.data_list)):
            label = self.data_list[i][1].split(',')[1].strip()
            label_id = dict[label]
            prob[i] = 1/len(cnt) * (1 / cnt[label_id])
        return prob
...
def nlg_get_data_loader(config, dataset, batch_size, shuffle=False):
    # weighted_sampler = WeightedRandomSampler(dataset.sample_prob,num_samples=len(dataset))

    collate_fn = lambda d: nlg_data_wrapper(config, d)

    # if config.sampling_type == "class_balance" and shuffle:
    #     return DataLoader(dataset, 
    #     batch_size=batch_size,
    #     num_workers=0,
    #     collate_fn=collate_fn,
