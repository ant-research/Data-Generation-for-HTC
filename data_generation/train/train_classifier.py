...
parser.add_argument('--lamb', default=0.05, type=float, help='lambda')
...
if __name__ == '__main__':
    ...
    if args.wandb:
        import wandb
        wandb.init(project="pda", entity="jordddan",name=args.name,job_type='training',reinit=True)
    ...
    for epoch in range(1000):
        ...
        for data, label, idx in pbar:
            ...
            max_len = padding_mask.sum(1).max()
            data = data[:, :max_len]
            ...
            i += 1
            if args.wandb:
                wandb.log({'learning_rate':optimizer.lr})
            if i % args.update == 0:
                optimizer.step()
                optimizer.zero_grad()
                if args.wandb:
