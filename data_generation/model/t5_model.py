...
class T5ForPT(T5PreTrainedModel):
    ...
    def forward(
        ...
        # new_active_logits = lm_logits[:lm_logits.size(0)//2]
        # active_logits = lm_logits[lm_logits.size(0)//2:]
        ...
        # loss += 1.0 *(kl_loss + reverse_kl_loss)/2
...
def get_full_finetune_t5_model(config):
    tokenizer = T5Tokenizer.from_pretrained(config.pre_trained_model)
    model = T5ForPT.from_pretrained(config.pre_trained_model, local_config=config)
