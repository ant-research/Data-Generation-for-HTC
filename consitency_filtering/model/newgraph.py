...
class BartAttention(nn.Module):
    ...
    def forward(
        ...
        if gumbel == True:
            attn_weights = F.gumbel_softmax(attn_weights, hard=False, dim=-1, tau=self.tau)
        else:
...
class GraphLayer(nn.Module):
    ...
    def forward(self, label_emb, extra_attn, self_attn_mask, inputs_embeds, cross_attn_mask, gumbel):
        if GRAPH == 'GRAPHORMER':
...
class GraphEncoder(nn.Module):
    def __init__(self, config, graph=True, layer=1, data_path=None, threshold=0.01, tau=1, args=None):
        ...
        self.label_embedding = nn.Embedding(len(self.label_name),embedding_dim=768)
    ...
    def forward(self, inputs_embeds, attention_mask, labels, embeddings,is_training=True):
        ...
        if self.args.rand_emb == True:
            label_id = torch.LongTensor([i for i in range(len(self.label_dict))]).to(inputs_embeds.device)
            label_emb = self.label_embedding(label_id)
        else:
            label_emb = embeddings(self.label_name)
        ...
        for hir_layer in self.hir_layers:
            label_emb = hir_layer(label_emb, extra_attn, self_attn_mask, inputs_embeds, cross_attn_mask, self.args.gumbel)
        ...
        # import pdb
        # pdb.set_trace()
        # if self.args.gumbel == True:
        #     contrast_mask = (F.gumbel_softmax(token_probs, hard=False, dim=-1, tau=self.tau) * labels.unsqueeze(1)).sum(-1)
        # else:
        #     contrast_mask = (F.softmax(token_probs, dim=-1) * labels.unsqueeze(1)).sum(-1)

        contrast_mask = (token_probs * labels.unsqueeze(1) / 2 ).sum(-1)

            # index = torch.topk(contrast_mask,dim=-1,k=10)
            ...
            # contrast_mask = torch.softmax(contrast_mask,dim=-1)
