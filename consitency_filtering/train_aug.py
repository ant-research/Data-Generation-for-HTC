...
class WOSAugDataset(Dataset):
    def __init__(self, max_token=512, device='cpu', pad_idx=0, data_path=None, tokenizer=None):
        
        self.device = device
        super(WOSAugDataset, self).__init__()

        label_dict = pickle.load(open('NLU_training_dataset/WebOfScience/label_dict.pkl','rb'))
        ...
        for line in lines:
            ...
            sentence = parts[0]
            l = parts[1].split(', ')
            s = [label_dict[l[0]], label_dict[l[1]]]

            self.data.append(torch.tensor(tokenizer.encode(sentence.strip().lower(), truncation=True)))

            one_hot = [0] * len(label_dict)
            for i in s:
                one_hot[i] = 1

            self.labels.append(torch.tensor(one_hot))
...
class AugDataset(Dataset):
    def __init__(self, max_token=512, device='cpu', pad_idx=0, data_path=None, tokenizer=None,task=None,num_class=None):
        
        self.device = device
        super(AugDataset, self).__init__()

        label_dict = pickle.load(open(os.path.join('NLU_training_dataset',task,'label_dict.pkl'),'rb'))
...
if __name__ == '__main__':
    ...
    if args.data == 'WebOfScience':
        aug_dataset = WOSAugDataset(device=device, pad_idx=tokenizer.pad_token_id, data_path=args.input_dir, tokenizer=tokenizer)    
    else:
        aug_dataset = AugDataset(device=device, pad_idx=tokenizer.pad_token_id, data_path=args.input_dir, tokenizer=tokenizer,task=args.data,num_class=num_class)
    ...
    print('aug number:',len(aug_dataset))
    ...
    train = DataLoader(aug_dataset, batch_size=args.batch, shuffle=True, collate_fn=dataset.collate_fn)
